...
abstract class GenericDAO<T> implements Serializable {
	...
	private static EntityManagerFactory emf = Persistence
			.createEntityManagerFactory("fazendola");
	...
	public T find(long l) {
		return em.find(entityClass, l);
	}

	public T findReferenceOnly(long l) {
		return em.getReference(entityClass, l);
	}
	...
	// em.getCriteriaBuilder().createQuery()<T> method
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public List<T> findAll(int posicaoAtual, int limite) {
		CriteriaQuery cq = em.getCriteriaBuilder().createQuery();
		cq.select(cq.from(entityClass));
		Query q = em.createQuery(cq);
		q.setMaxResults(limite);
		q.setFirstResult(posicaoAtual);
		return em.createQuery(cq).getResultList();
	}
